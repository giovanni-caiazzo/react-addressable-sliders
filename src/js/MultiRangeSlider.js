...
const MultiRangeSlider = ({
  ...
  const [extremes, setExtremes] = useState(
    init_extremes || getMinAndMaxFromRanges(init_ranges)
  );
    ...
    <div style={styles.container}>
      <div
        ...
        onClick={(event) =>
          emptySpaceCallback
            ? emptySpaceCallback(
                getClickedValueOnTrack(event, extremes, ranges)
              )
            : null
        }
      >
        {Object.values(ranges).map((range) => (
          <div key={range.id}>
            {["min", "max"].map((thumb) => {
              ...
              const disabled =
                (options.hasOwnProperty("isImmovable") &&
                  options.isImmovable(range)) ||
                (range.immovable && range.immovable[thumbPosition]);
              return (
                <div
                  key={`${range.id}_${thumb}`}
                  onMouseEnter={() =>
                    toggleTooltip("show", range, thumb, labelsRef, extremes)
                  }
                  onMouseLeave={() =>
                    toggleTooltip("hide", range, thumb, labelsRef, extremes)
                  }
                >
                  <input
                    type={"range"}
                    ...
                    style={
                      !disabled
                        ? styles.thumb
                        : { ...styles.disabledThumb, ...styles.thumb }
                    }
                  />
                  <span
            ...
            >
              <span
                style={styles.sliderNamePopup}
                ref={(el) => (namesRef.current[range.id] = el)}
              >
                {range.name}
              </span>
