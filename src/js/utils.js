...
export const onThumbValueChange = (
  ...
  if (thumb === "min") {
    ...
    newValue = Math.max(
      value,
      nearestRangeValue + (options?.rangePadding || 0)
    );
  } else {
    ...
    newValue = Math.min(
      value,
      nearestRangeValue - (options?.rangePadding || 1)
    );
  }
  if (emitChanges) {
  ...
  range_intervals.forEach((considered_range) => {
    ...
    range_intervals.forEach((range) => {
      if (considered_range.id !== range.id) {
        if (
          considered_range.min < range.max &&
          considered_range.min > range.min
        ) {
          considered_range.min = range.max + rangePadding;
        }
        if (
          considered_range.max < range.max &&
          considered_range.max > range.min
        ) {
...
export const toggleTooltip = (action, range, thumb, labelsRef, extremes) => {
  if (labelsRef.current && labelsRef.current[range.id]) {
    ...
    labelsRef.current[range.id][label_position].style.visibility =
...
export const toggleNameTooltip = (action, range_id, namesRef) => {
  if (namesRef.current && namesRef.current[range_id]) {
    namesRef.current[range_id].style.visibility =
      action === "show" ? "visible" : "hidden";
    namesRef.current[range_id].style.left =
      action === "show"
        ? `calc(50% - ${namesRef.current[range_id].clientWidth / 2}px) `
        : 0;
  }
};
...
export const getClickedValueOnTrack = (event, extremes, ranges) => {
  ...
  const relativeValue = parseInt(
    extremes.min + (relativePercentage / 100) * (extremes.max - extremes.min)
  );
  let closestRanges = { min: relativeValue, max: relativeValue };
  Object.values(ranges).forEach((range) => {
    if (range.max < closestRanges.min) {
      closestRanges.min = Math.max(
        range.max,
        closestRanges.min === relativeValue ? range.max : closestRanges.min
      );
    }
    if (range.min > closestRanges.max) {
      closestRanges.max = Math.min(
        range.min,
        closestRanges.max === relativeValue ? range.min : closestRanges.max
      );
    }
  });
  return {
    ...
    closestRanges: {
      min:
        closestRanges.min === relativeValue ? extremes.min : closestRanges.min,
      max:
        closestRanges.max === relativeValue ? extremes.max : closestRanges.max,
    },
  };
};
